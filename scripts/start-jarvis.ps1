# Jarvis Voice Local - friendly one-click launcher (v1.0 Slice 1).
#
...
#
# -AppWindow opens JARVIS as a clean Edge "app" window (no browser chrome) instead of a
# normal browser tab; this is what the pinnable JARVIS shortcut uses. Falls back to the
...
# 1) Find a Python 3.10+ (Jarvis needs it). Prefer one on PATH; otherwise scan the
...
$pyExe = Get-AppPython
if (-not $pyExe) {
    Say "Jarvis needs Python 3.10 or newer, and I couldn't find it." "Red"
...
foreach ($f in @("server.py", "config.py")) {
    if (-not (Test-Path (Join-Path $root $f))) {
        Say "Missing $f - run this from inside the jarvis-voice-local folder." "Red"
        exit 1
    }
}
...
# Read JARVIS_TOKEN from .env (read-only; same parsing as scripts\copy-token.ps1) so we
...
} catch { $jarvisToken = $null }
if (-not [string]::IsNullOrWhiteSpace($jarvisToken) -and $jarvisToken -ne "replace-with-a-long-random-token") {
    $openUrl = "$localUrl#token=" + [uri]::EscapeDataString($jarvisToken)
}

# Start the high-quality Jarvis voice (Kokoro TTS) on 127.0.0.1:8001 if it's installed
...
if (-not $ttsUp -and (Test-Path $ttsPy) -and (Test-Path $ttsModel)) {
    Say "Starting the Jarvis voice (Kokoro)..." "DarkGray"
...
if ($alreadyUp) {
    Say "Jarvis is already running on port $port (v$($h.version), mode $($h.agent_safety.mode))." "Green"
...
Say "Starting Jarvis Voice Local on $localUrl ..." "Cyan"
# Title the server window "Jarvis Voice Local" so users recognize it and it matches the
# update/setup instructions ("close the black window titled 'Jarvis Voice Local'"). The
# backtick escapes `$Host so it's evaluated in the NEW window, not here at build time.
$serverCmd = "`$Host.UI.RawUI.WindowTitle = 'Jarvis Voice Local'; Set-Location -LiteralPath '$root'; & '$pyExe' -m uvicorn server:app --host $($c.host) --port $port"
try {
...
if ($ready) {
    Say "Jarvis is UP (v$($h.version), mode $($h.agent_safety.mode))." "Green"
    Say "Opening $localUrl (signed in automatically)" "Cyan"
    Open-App $openUrl
    Say ""
    if ($openUrl -eq $localUrl) {
        Say "Note: JARVIS_TOKEN wasn't found in .env, so the app opened without signing you in." "Yellow"
    ...
    Say "(Common cause: JARVIS_TOKEN missing or Claude not found - doctor will say which.)" "DarkGray"
    exit 1
}
