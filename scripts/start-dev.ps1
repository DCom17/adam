# Jarvis Voice Local - start the backend (dev).
...
if (-not (Test-Path (Join-Path $root ".env"))) {
    Write-Host "WARNING: no .env found. Copy .env.example to .env and set JARVIS_TOKEN." -ForegroundColor Yellow
}

Write-Host "Starting Jarvis Voice Local on http://$($c.host):$($c.port) ..." -ForegroundColor Cyan
