# Adam - copy your ADAM_TOKEN to the clipboard (no manual .env editing).
#
...
# into the Adam app's settings field.
#
# It is intentionally minimal and safe:
#   * READ-ONLY: it reads .env to find ADAM_TOKEN and nothing else;
...
if (-not (Test-Path -LiteralPath $envFile)) {
    ...
    Say "(That generates your ADAM_TOKEN. Then re-run this helper.)" "DarkGray"
    exit 1
}

# 2) Find ADAM_TOKEN (skip comments; tolerate spaces and optional quotes).
# JARVIS_TOKEN is accepted too: pre-rename installs still carry it in .env.
$token = $null
...
if ([string]::IsNullOrWhiteSpace($token) -or $token -eq "replace-with-a-long-random-token") {
    Say "ADAM_TOKEN is not set in .env (missing, blank, or still the placeholder)." "Yellow"
    Say "Re-run setup to generate one:  python scripts\setup.py" "Cyan"
    Say "If you just ran setup, open .env and confirm a ADAM_TOKEN= line has a real value." "DarkGray"
    exit 1
}
...
if (-not $copied) {
    ...
    Say "As a fallback, open .env yourself and copy the ADAM_TOKEN value." "Yellow"
    exit 1
}
...
Say "Copied ADAM_TOKEN to clipboard." "Green"
Say "  (token: $len chars, ends with '$last4' - the value is NOT shown here)" "DarkGray"
Say "Paste it into the Adam app settings." "Cyan"
