' Adam app launcher (pinnable). This is the target of the "Adam" shortcut.
'
' It runs start-adam.ps1 -AppWindow with no console window of its own, so clicking
' the pinned Adam orb: starts the server if it isn't running (the server still opens
' its OWN visible window, per Adam's transparent design), waits for it, then opens a
...
ps1  = fso.BuildPath(here, "start-adam.ps1")
