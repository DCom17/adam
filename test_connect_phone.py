"""
Jarvis Voice Local — connect-phone helper tests (v1.0 Slice 2).
...
Proves the helper is read-only, safe, and recommends the right HTTPS port:
  ...
  * :443 occupied (Morrow or other) -> recommends :8443; :443 free -> :443;
  * generated command targets the local Jarvis port and NEVER contains reset/funnel;
  * output warns not to run `serve reset` when a serve exists;
  * no JARVIS_TOKEN in human or JSON output; subprocess failures are handled;
...
def main() -> int:
    ...
    try:
        ...
        mi = cp.parse_serve_status(MORROW_SERVE)
        check("Morrow serve -> :443 in use", mi["port_443_in_use"] is True)
        check("Morrow serve -> morrow_detected", mi["morrow_detected"] is True)
        oi = cp.parse_serve_status(OTHER_443_SERVE)
        check("other :443 serve -> :443 in use", oi["port_443_in_use"] is True)
        check("other :443 serve -> not morrow", oi["morrow_detected"] is False)
        ...
        j = cp.parse_serve_status(JARVIS_8443_SERVE)
        check("jarvis :8443 serve parsed", 8443 in j["served_https_ports"] and not j["port_443_in_use"])
        ...
        r = with_env("C:/fake/onpath/tailscale.exe", MORROW_SERVE)
        d = cp.gather()
        ...
        check("morrow detected -> recommend 8443", d["recommended_https_port"] == 8443)
        ...
        print("\n[4] :443 occupied by something non-Morrow -> still :8443")
        with_env("C:/fake/tailscale.exe", OTHER_443_SERVE)
        d = cp.gather()
        check("non-morrow :443 -> recommend 8443", d["recommended_https_port"] == 8443)
        check("morrow_detected False", d["morrow_detected"] is False)
        ...
        with_env("C:/fake/tailscale.exe", MORROW_SERVE)
        d = cp.gather()
        blob = json.dumps(d)
        ...
        runner = with_env("C:/fake/tailscale.exe", MORROW_SERVE)
        cp.gather()
        mutating = []
        ...
        _tok = config.JARVIS_TOKEN
        try:
            config.JARVIS_TOKEN = "SENTINEL_TOKEN_DO_NOT_LEAK_abc123"
            with_env("C:/fake/tailscale.exe", MORROW_SERVE)
            d = cp.gather()
            human = io.StringIO()
            ...
            check("no 'JARVIS_TOKEN' key/value in diagnostic", "JARVIS_TOKEN" not in json.dumps(d))
        finally:
            config.JARVIS_TOKEN = _tok
        ...
        def make_d(**over):
            d = {
                ...
                "morrow_detected": True, "recommended_https_port": 8443,
        ...
        rr = rec_runner(); cp.subprocess.run = rr
        rc = cp._do_apply(make_d(recommended_https_port=443, served_https_ports=[443], morrow_detected=True), assume_yes=True)
        check("would-clobber :443 (Morrow) -> refused", rc == 1 and rr.calls == [])

        print("\n[13] --apply happy path runs exactly one `serve --bg`")
        rr = rec_runner(); cp.subprocess.run = rr
        rc = cp._do_apply(make_d(), assume_yes=True)   # signed in, 8443 free (Morrow on 443)
        ...
        print("\n[15] --off is port-scoped and refuses :443 when it's Morrow")
        rr = rec_runner(); cp.subprocess.run = rr
        rc = cp._do_off(make_d(morrow_detected=True), 443, assume_yes=True)
        check("off :443 with Morrow -> refused (exit 1)", rc == 1)
