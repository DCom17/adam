...
SERVE_JSON = json.dumps({
    "TCP": {"8443": {"HTTPS": True}},
    "Web": {
        "examplebox.tail0000ab.ts.net:8443": {
...
SERVE_JSON_OTHER = json.dumps({
    "Web": {"examplebox.tail0000ab.ts.net:443": {"Handlers": {"/": {"Proxy": "http://127.0.0.1:9100"}}}}
})
...
def main() -> int:
    ...
    check("serve url matches this app's port",
          pl.parse_serve_url(SERVE_JSON, 8010) == "https://examplebox.tail0000ab.ts.net:8443")
    ...
    check("dns name parsed + trailing dot stripped",
          pl.parse_dns_name(STATUS_JSON) == "examplebox.tail0000ab.ts.net")
    ...
    check("tailnet url not local", not pl._is_local("https://examplebox.tail0000ab.ts.net:8443"))
    ...
    try:
        ...
        check("tailscale https url is a candidate",
              "https://examplebox.tail0000ab.ts.net:8443" in urls)
        ...
        check("best is the secure (HTTPS) tailscale url",
              best and best["url"] == "https://examplebox.tail0000ab.ts.net:8443" and best["secure"])
